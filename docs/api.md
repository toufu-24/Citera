...
Base path は `/v1`。Structured request は route ごとの Zod schema または `packages/sync` の共有 schema で検証します。Domain response schema もありますが、現在の API は全 response を runtime Zod parse しているわけではありません。Web/extension はそれぞれの client adapter を使います。

## Authentication

Web は random session token を `HttpOnly; SameSite=Lax` cookie で受け取ります。本番だけ `Secure` が付きます。拡張は `Authorization: Bearer <short-lived-access-token>` を使います。API は認証結果から得た `userId` だけを使い、request body/query の user ID を authority として信頼しません。
...
| DELETE | `/files/:fileId`                    | Soft delete and enqueue cleanup                                             |
...
`kind` defaults to `original_pdf`; `ingestionId` is optional for a PDF added outside an ingestion.
