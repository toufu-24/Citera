...
本番の静的 Web と identity-bootstrap endpoint は Cloudflare Access で保護します。`/v1/auth/session` が Access の `Cf-Access-Jwt-Assertion` を署名・issuer・audience・有効期限まで検証し、`access_issuer + access_subject` で利用者を識別して Citera HttpOnly session cookie を発行します。API path 自体は Access Bypass ですが、以後の全 route はその cookie または拡張機能 bearer を Worker で認証します。初回認証時に `users`、個人 `libraries`、owner membership を作ります。APIは request body/query の user ID や library ID を authority として信頼しません。
...
Extension authorize endpoint requires an authenticated Access request in production (or an existing Web session locally). Its `redirect_uri` must match `ALLOWED_EXTENSION_IDS`-derived Chromium callbacks or the explicit `EXTENSION_REDIRECT_ORIGINS` allowlist; token exchange then uses the one-time code and PKCE verifier.
...
| GET    | `/auth/login/google`        | Local legacy Google OAuth; disabled in production                      |
| GET    | `/auth/callback/google`     | Local legacy callback; disabled in production                          |
...
- Production Web identity providers are configured in Cloudflare Access; Citera does not implement a production social-login callback.
